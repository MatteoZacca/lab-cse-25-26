...
object WordCount {
  ...
  val outputDir = "/output/myFirstWordCount" // The output directory should NOT exist, so
  // it means that if you run the same job multiple times you have to delete this directory.
  // you don't have to create it, it gets created automatically
  ...
  def main(args: Array[String]): Unit = {
    ...
    myRdd.
      ...
      map({case (k,v) => (v,k)}).
      sortByKey(false).
      coalesce(1). // coalesce did one -> the results are grouped into a single partition
