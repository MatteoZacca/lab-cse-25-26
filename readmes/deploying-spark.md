...
If the configuration worked perfectly, this command will return a JSON with three fields: UserId, Account and Arn
...
- Check the main bucket:
  ```bash
  aws s3 ls s3://unibo-bd2526-mzaccarini/
  ```
  
- Check inside the datasets folder:
  ```bash
  aws s3 ls s3://unibo-bd2526-mzaccarini/datasets/
  ```

- Stream the content of a text file:
  ```bash
  aws s3 cp s3://unibo-bd2526-mzaccarini/datasets/capra.txt -
  ```
